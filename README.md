...
## 1. Abstract
While other Neural Networks require large datasets to learn patterns, **Physics-Informed Neural Networks (PINNs)** essentially utilize physical laws into the learning process. This project implements a PINN to solve the **1D Burgers' Equation**—a fundamental Partial Differential Equation (PDE) in fluid mechanics—demonstrating how AI can obey conservation laws even with minimal data.
...
* **Composite Loss:** The model minimizes both data error and the physics error:
...
## 6. Conclusion
I developed this project to explore application of computation in physics with an inspirational ambition of pursuing AI for Science. It shows that we can use AI not just to recognize patterns, but to model and solve complex physical systems. 
